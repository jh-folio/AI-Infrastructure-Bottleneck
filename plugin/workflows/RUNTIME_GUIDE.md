...
HTML은 대시보드와 공급망 지도 두 화면으로 제공한다. 대시보드는 검토된 구간·공통 점수 또는 범위·방향을 간결하게 표시하고 클릭 설명과 6M Trend/Evidence/Companies를 연다. 점수 미평가는 —, 비교 불가는 ·, 잠정 점수는 *로 표시한다. 확정 단일값이 아닌 범위를 중간값으로 바꾸지 않는다. 방향 가속을 입증하는 계약이 없으므로 임의의 두 단계 화살표를 만들지 않는다.

6M Trend는 같은 노드·규격·지역·시나리오·평가기간·방법의 과거 보고서 점수 중 최근 6개월만 표시한다. 정정된 근거나 같은 날짜의 서로 다른 판단은 선으로 연결하지 않는다. 이력이 부족하면 추세선 대신 미확인을 보여 준다. 날짜 간 빈 구간을 새 관측값으로 채우지 않는다.
