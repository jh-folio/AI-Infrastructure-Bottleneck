...
**현행 범위·비용 계약(d5.11):** [효율적인 조사](EFFICIENT_RESEARCH.md)를 적용한다. 새 campaign의 기본 조사 대상은78개이며 인력/인허가10개는 필요한 맥락 요인으로 확인한다. 기존87개 ID·정의는 보존하고 기존 campaign은 명시적 전환 전까지 당시 범위를 유지한다. 전수 조사와 완료 판정은 campaign scope_profile에 포함된 유효 노드 전체에 적용한다. 78/87/88 같은 숫자를 완료 조건으로 고정하지 않는다.
...
닫는 질문에는 `source_reviews` 배열을 남긴다. 실제로 읽은 자료의 document_id/location/quote와 해당 질문에서 알아낸 finding, 노드·규격·지역·시점에 적용되는 이유 relevance, 자료 역할 role을 기록한다. 원문 하나로 여러 항목을 검토할 수 있다. **모든 질문마다 새 자료·새 다운로드를 요구하지 않는다.** 출처 전체를 반복 요약하지 말고 각 질문에 답하는 부분만 재조회한다.
...
변경 뒤 research-resume/next로 새 노드를 조사한다. retired 질문은 역사 기록으로 보존하지만 현재 완료 대기열에서는 빼고 새 유효 노드의 모든 질문을 요구한다. 전체 최초 조사 대상은 campaign scope_profile에 포함된 현재 유효 노드 전부다. 추가·분할·통합된 노드도 범위 계약에 따라 조사 대기열에 포함한다. baseline_review는 retired 노드와 후속 노드를 함께 현재 평가에 포함하지 못한다. 과거 보고서/지도는 고정 snapshot을 유지한다. 분할/통합 계보는 기술적 공급 연결선이 아니므로 지도 관계를 자동 생성하지 않는다.
