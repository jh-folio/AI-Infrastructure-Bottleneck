...
최초 보고서 요청은 현재 유효한 모든 노드의 실제 조사와 종합이다. 이를 에이전트가 '목록 검토', '중간 기준선', '기능 시험'으로 축소하지 않는다. **중간 파일 저장은 내부 작업이며 사용자 요청의 완료가 아니다.** 아래 순환을 같은 실행에서 이어간다. 이번 보완에서는 여러 실행 분할·예약·다음 사용자 메시지를 전제로 하지 않는다.
...
research-resume은 미탐색 노드·빠진 질문·open/blocked와 출처 확인/반복 내용 문제를 반환한다. pending이 남으면 내부 저장 뒤 같은 실행에서 다음 조사를 수행한다. 오래 걸리면 진행 범위와 다음 행동을 알리고 계속한다. 실제 호스트 중단·도구 사용 불가·사용자 중지로 계속할 수 없을 때만 미완료 상태와 관측된 장애를 전달한다. 그때도 연구 완료·전수 조사 성공이라고 쓰지 않는다. 미래 실행을 자동 약속하거나 임의 예약하지 않는다.
