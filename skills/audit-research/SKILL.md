...
[연구 실행 계약](../../plugin/workflows/RESEARCH_EXECUTION.md)의 source_reviews와 research-completion을 적용한다. 읽기 전용 완료 확인에서 미조사·원문 불일치·반복 결론이 반환되면 실제로 검토할 질문을 제시한다. 해시 검사 통과나 조사 대상 수만큼의 행 존재만으로 전수 조사라고 인정하지 않는다. 이름만 바꾸어도 성립하는 설명, 산업 배경을 노드 부족으로 바꾼 결론, 현재값을 역사·추세로 반복한 답변은 원문과 대조한다.
