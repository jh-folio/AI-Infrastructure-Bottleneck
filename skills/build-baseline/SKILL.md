...
최초 연구는 research-start, 재개는 research-resume으로 시작한다. 기존 87개 노드 목록을 빠뜨림 확인에 사용하고 조사 범위를 설명한다. 선정 노드에는 수요/가용 공급·과거 계획과 실현·경쟁 가설/반증·가동 영향·추세 질문을 구체화한다. 질문 묶음별 원문을 저장하고 checkpoint로 다음 행동을 남긴다. '미확인'을 쓰고 종료하지 말고 판단을 바꿀 자료를 찾아 후속 조사한다.
...
research-resume의 pending이 남으면 후속 조사하거나 중간 결과와 정확한 재개 위치를 저장한다. 시간·문서 개수·점수 개수로 종료하지 않는다. synthesize 기본 interim은 중간 결과다. baseline_review는 최신 campaign/checkpoint와 실제 연구 수행 기록을 연결하고, 종료 기준을 충족한 뒤에만 제출한다. 코드 통과만으로 연구 품질·사용자 인수를 선언하지 않는다.
